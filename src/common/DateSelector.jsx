...
const Week = props => {
  ...
  return (
    <tr className="date-table-days">
      {
        days.map((day, index) => (
          <Day key={index} onSelect={onSelect} day={day} />
        ))
      }
    </tr>
  )
...
const Month = props => {
  ...
  // 获取一个月的具体每天0时刻日期，并补齐在日历中前后缺失的空白天（一个月的开头与结尾并不总是在周一和周末的）
  ...
  days = days.concat(new Array(lastDay.getDay() ? 7 - 1 : 0).fill(null))
      ...
      <tbody>
        ...
        </tr>
        {
          weeks.map((week, index) => (
            <Week key={index} onSelect={onSelect} days={week} />
          ))
        }
      </tbody>
    </table>
